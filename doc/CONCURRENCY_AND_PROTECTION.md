...
> Дополнение к `DEVELOPMENT_PLAN.md`. Реализовать как следующий этап после деплоя.
...
- [ ] Семафор в `imagen.py` на `MAX_CONCURRENT_API_CALLS` (все вызовы generate_content для картинок)
- [ ] Lock на сессию — не запускать генерацию если предыдущая ещё идёт
- [ ] Lock на `user_data.json` при записи
- [ ] Whitelist `ALLOWED_USERS` в `.env`, декоратор `@authorized`
- [ ] Дневной лимит генераций в `session.py`
- [ ] Логирование: кто, какая модель, сколько вариантов, время генерации
