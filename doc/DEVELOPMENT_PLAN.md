...
## Реализованный этап — BOT_TOV (тон и UX бота)

Подробности в `BOT_TOV.md`. Все пункты выполнены:

- [x] Реакция 👀 на скетч — мгновенный «кивок» при получении фото (🌿 недоступен в Bot API)
- [x] `StatusMessage` — одно сообщение с анимацией точек, edit-in-place, две фазы: `📝 пишу промпт` → `🎨 генерирую`, удаление после генерации
- [x] Все сообщения бота в моноспейсе (Markdown backticks)
- [x] Ошибки — короткие, моноспейс, формат «факт — действие» (`` `фильтр: текст — переформулируй` ``)
- [x] `/start` минимализм — `🌱 новый проект` вместо простыни
- [x] Подсказки как inline-кнопки под сеткой (callback `suggest:N`, текст из `session.suggestions`)
- [x] Нажатие подсказки = новая итерация с текстом подсказки как гипотезой
- [x] Системные промпты обновлены: вопросы вместо директив, правила для подсказок
